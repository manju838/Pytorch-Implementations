...
for epoch in range(num_epochs):
    for batch_idx, (data, targets) in enumerate(train_loader): #enumerate creates an array like mapping with each entry in train_loader(here the tuple(data,targets) ) indexed to a no.(here batch_idx) 
        # Get data to cuda if possible,i.e transfering data and targets to GPU if possible as device can be either CPU or GPU selected here 
